...
class AntibodyParser(PDBParser):
    ...
    def __init__(self, PERMISSIVE=True, get_header=True, QUIET=False, scheme="chothia", definition="chothia"):
        ...
        assert scheme.lower() in ["c", "a", "k", "imgt", "chothia", "martin", "kabat",
                                  "wolfguy"], "Unrecognised scheme: %s" % scheme
        self.numbering_scheme = scheme.lower()

        assert definition.lower() in ['c', 'n', 'k', 'imgt', 'chothia', 'north', 'kabat',
                                      "wolfguy"], "Unrecognised definition: %s" % definition
    ...
    def set_region_definition(self, definition="chothia"):
        ...
        Otherwise use chothia
        """
        assert definition.lower() in ['c', 'n', 'k', 'imgt', 'chothia', 'north',
                                      'kabat'], "Unrecognised definition: %s" % definition
        self.region_definition = definition.lower()
    ...
    def get_antibody_structure(self, id, file):
        """
            ...
            for chain in model.get_list():
                ...
                if self.numbering_method == "user":
                    if chain.id == self.VH_id:
                        ...
                        force = "L"
                try:
                    numbering, chain_type = annotate(chain, self.numbering_method, force,
                ...
                newchain = self._create_ab_chain(chain, chain.id, numbering, chain_type, self.numbering_scheme,
        ...
        for residue in chain.get_list():
            ...
            if residue.id in numbering:
                if numbering[residue.id]:
                    add = True
                    res_id = (
                        residue.id[0], numbering[residue.id][0],
                        numbering[residue.id][1])  # field and reannotated things.
        ...
        for pair in combinations(chains, 2):
            if pair[0].chain_type != pair[1].chain_type:
                try:
                    ...
                    print(
                        "Warning: Antibody chain does not contain either residue H%d or L%d. Pairing will fail for chain %s" % (
                            points['H'][1], points['L'][1], pair[0].id), file=self.warnings)
                    continue
                if a1 - a2 < 22:
    ...
    def _match_antigens(self, model, abchains, agchains, crystal_contacts=[]):
        ...
        for antigen in agchains:
            antigen_atoms[antigen.id] = [a for a in antigen.get_atoms() if a.parent.id[0] == " " or is_aa(
                a.parent)]  # test ATOM records or amino acid HETATM records
            antigen_hetatoms[antigen.id] = [a for a in antigen.get_atoms() if
                                            a.parent.id[0].startswith("H") and not is_aa(
                                                a.parent)]  # hetatm and not an amino acid
        ...
        ns = NeighborSearch(
            [atom for chain in cdr_atoms for atom in cdr_atoms[chain]] + [atom for chain in antigen_atoms for atom in
                                                                          antigen_atoms[chain]])
        ...
        for ag in ags:
            ...
            if cmax:
                if len(contact_freq) > 1:
                    self.warnings.write(
                        "Crystal Contact Warning: antigen %s has been paired with fab %s" % (str(ag), str(paired_fab)))
    ...
    def _check_het_antigen(self, residue):
        ...
        if is_common_buffer(residue):
            if not self.QUIET:
                print(
                    "Common molecule %s found in the binding site - not considered an antigen" % residue.get_resname(),
                    file=self.warnings)
            return False
    ...
    def _analyse_header(self, header):
        ...
        for compound in header["compound"]:
            ...
            if "chain" in header["compound"][compound]:
                ...
                chains = [c.strip().upper() for c in header["compound"][compound]["chain"].split(",") if
                          len(c.strip()) == 1]
                ...
                if "engineered" in header["compound"][compound]:
                    if "no" in header["compound"][compound]["engineered"] or "false" in header["compound"][compound][
                        "engineered"] or not header["compound"][compound]["engineered"]:
                ...
                if a in atomid_to_resid:
                    try:
                        r_connections[atomid_to_resid[a]].update(
                            [atomid_to_resid[ai] for ai in connect_records[a] if ai in atomid_to_resid])
                    except KeyError:
